          ...
          <table class="vol-kpis" role="presentation">
            <tr>
              ...
              <td class="kpi vol-total" width="33%">
                <p class="kpi-label">Traded Lots</p>
...
function buildEmailHtml({ fromYmd, toYmd, rows, volume, charts = null, chartError = null }) {
      ...
      <div class="wrap">
        ...
        <div class="content">
          ...
          <div class="foot">
            ...
            Traded Lots come from ClientVolume/Run (the dashboard's Dealing (LP) volume tile). Total Lots above come from DealMatch/Run &mdash; a different measure, so the two do not reconcile.
          </div>
        </div>
      </div>
    </div>
...
export async function runWeeklyDealMatchEmailReport({ fromDate, toDate, recipients: recipientsOverride } = {}) {
  ...
  const html = buildEmailHtml({ fromYmd, toYmd, rows, volume, charts: chartUrls, chartError });
