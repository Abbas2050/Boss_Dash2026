...
function toUnixRange(fromDate, toDate) {
      ...
      <div class="wrap">
        ...
        <tfoot>
          ...
          <div class="foot">
            ...
            Formula: Net Revenue = (Markup + Client Comm) - (IB Commission - LP Comm)
          </div>
        </div>
      </div>
    </div>
...
async function sendBrevoEmail({ subject, html, recipients, attachments = [] }) {
...
async function renderChartBuffer(config, width = 1200, height = 700) {
...
export async function runWeeklyDealMatchEmailReport({ fromDate, toDate } = {}) {
  ...
  const rows = await mapWithConcurrency(
    ...
    async (row) => {
      ...
      const netRev = (markup + clientComm) - (ibCommission - lpComm);
      return {
        ...row,
        totalRev,
        ibCommission,
